...
실행하는 디버깅 도구다. runner 가 저장한 ``{stem}_result.json`` 과 (있으면) 점군 PLY 를
읽어 rerun 으로 로깅하고 ``.rrd`` 파일을 남긴다.

그리는 것:
  - target 점군 (``{stem}_with_anchor.ply``, 있으면) — 회색 배경
  - anchor L/R/U (Points3D, 색 구분) — result.json 의 정확한 좌표
  - plane normal (Arrows3D, anchor 중심에서)
  - safe zone OBB (Boxes3D) — 통과 anchor 는 초록, 위반 anchor 는 빨강
  - 위반 anchor 강조 (빨강 큰 점)

좌표 프레임: result.json 의 anchor / safe_zones 는 **카메라 프레임**(teaching 정의 그대로).
T_cam_to_base 가 있으면 로봇 프레임 비교도 가능하지만, 시각화는 카메라 프레임에서 한다
(파이프라인 검사와 동일 — anchor 와 safe zone 을 같은 프레임에서 비교).

설치:
    uv sync --extra viz          # rerun-sdk

실행:
    # result.json 직접 지정
    uv run python -m crp_matching.utils.rerun_viz \
        path/to/undistorted_5_match_depth_result.json

    # 디렉터리 안의 *_result.json 전부
    uv run python -m crp_matching.utils.rerun_viz path/to/data_dir --glob

    # 즉시 뷰어 띄우기 (WSL 에서 GUI 되면)
    uv run python -m crp_matching.utils.rerun_viz result.json --spawn
...
# 패키지 내부 헬퍼 — safe zone OBB 파라미터 계산.
...
def _parse_calibration_matrix(value):
    """T_cam_to_base 값 → 4x4 numpy. 중첩 리스트/길이16 모두. 형식 틀리면 None."""
    if value is None:
        return None
...
def _require_rerun():
    try:
        ...
        sys.exit(
            "rerun 이 설치되어 있지 않습니다. `uv sync --extra viz` 로 설치하세요."
        )
    return rr


def _rotation_matrix_to_quaternion_xyzw(R: np.ndarray) -> List[float]:
    """3x3 회전 행렬 → quaternion [x, y, z, w] (rerun Boxes3D 용).

    scipy 가 있으면 그걸 쓰고, 없으면 순수 numpy fallback.
    """
    try:
        from scipy.spatial.transform import Rotation
        return Rotation.from_matrix(R).as_quat().tolist()  # scipy: xyzw
    except Exception:
        # numpy fallback (Shepperd's method 간소화).
...
def _backproject_depth(depth_path: str, K: np.ndarray, texture_path: Optional[str]):
    ...
    ``_with_anchor.ply`` 에는 anchor 가 sphere 로 섞여 있어 헷갈리므로, 원본 depth 를
    직접 backproject 한다. depth tif 는 3채널(grayscale 복제)일 수 있어 채널 0 사용.
    texture 가 있으면 픽셀 색을 입힌다. 좌표/단위는 PLY 와 동일(mm).
    """
    try:
        import tifffile
    except ImportError:
...
def visualize(json_path: str, *, spawn: bool = False, save: bool = True) -> Optional[str]:
    ...
    # 캘리브레이션: teaching(safe_zone 출처) / runtime(anchor·ply 출처) 구분.
    # 둘 다 없으면 T_cam_to_base 로 fallback (self-match 호환).
    # (numpy 배열은 `or` 로 fallback 불가 — 명시적 None 체크.)
    _T_fallback = _parse_calibration_matrix(result.get("T_cam_to_base"))
    ...
    # ── camera frame 탭: 매칭(runtime) 카메라 기준. anchor·ply 만(원본 좌표). ──
    ...
    # ── robot frame 탭: anchor·ply→T_runtime, safe_zone→T_teach 로 모아 비교. ──
    ...
    summary = [f"frame: {stem}", f"status: {status}",
               f"frames: cam" + ("  +  robot" if have_robot else "  (no T_cam_to_base)")]
...
def _log_frame(rr, prefix, anchors, safe_zones, bad_point, violation,
    ...
    """한 프레임(prefix)에 로깅 — anchor/ply 와 safe_zone 에 **다른 변환** 적용.
    ...
    for name in ("L", "R"):
        ...
        if Tz is None:
            ...
            center, half, R = transform_safe_zone(zone, Tz)
        ...
        rr.log(f"{prefix}/safe_zone/{name}",
               rr.Boxes3D(centers=[np.asarray(center).tolist()],
    ...
    if bad_point and safe_zones.get(bad_point) is not None:
        ...
        if bad_pos is not None:
            if Tz is None:
                zc, zh, zR = safe_zone_to_obb(safe_zones[bad_point])
            else:
                zc, zh, zR = transform_safe_zone(safe_zones[bad_point], Tz)
