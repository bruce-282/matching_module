#!/usr/bin/env bash
#
# 매칭 실행 스크립트 (이 레포 run_matcher.py).
#
# 기본 config : configs/NX4/matcher_config.yaml        (input_dir 등 런타임 설정)
# 기본 template: configs/NX4/hood.6.matching_model.config.yml
#
# 사용법:
#   bash scripts/shell/run_match.sh            # 매칭만
#   VIZ=1 bash scripts/shell/run_match.sh      # 매칭 후 rerun .rrd 생성 (pip install -e .[viz])
#
...
echo "[run_match] python   : $PY"
echo "[run_match] cuda alloc: $PYTORCH_CUDA_ALLOC_CONF"
echo "[run_match] config   : $CONFIG"
echo "[run_match] template : $TEMPLATE"
...
if [ "${VIZ:-0}" = "1" ]; then
  echo "[run_match] rerun viz: $OUT_DIR (*_result.json -> .rrd)"
  "$PY" -m core.utils.rerun_viz "$OUT_DIR" --glob
fi

echo "[run_match] done. outputs -> $OUT_DIR"
